...
// Run all 100 supply curves for a given facility
// 10 treatmeants X 10 systems
...
const run = async () => {
  ...
  // nested loop over all treatments and systems
  for (const treatmentId of treatments) {
    for (const harvestSystem of systems) {
      ...
      fs.writeFileSync((process.env.CSV_DIR || './data/') + fileName, fileContents);
    }
  }
};
